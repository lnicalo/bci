function [acc, out, scores, tiempo] = SSSRKDApredict3( features, gnd, model )
% features = repmat(features, [3 1 1]);
% gnd = repmat(gnd,[1 3]);
...
for i = Ntrain+1 : (Ntrain + Ntest)
    t = tic;
    [labels_n(1, i), post_n(1, i)] = SRKDApredict_(K(i, post > th), model);
