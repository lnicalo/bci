...
rootPath = '../../02Data/';
dataset = 'competIVdatasetIIa2_310foldCV';
extrFeatAlg = 'FBCSP';
featSelAlg = 'MIBIF';
...
%% Plot
% Convert ind -> several indexes
ind_ = cell(length(Lparam),1);
[ind_{1:length(Lparam)}] = ind2sub(Lparam', ind);

for j = 1:length(Lparam)
    plot1 = figure;
    
    index = cell(length(Lparam),1);
    for k = 1:length(Lparam)
        if k ~= j
            index{k} = ind_{k};
        else
            index{k} = 1:Lparam(k,1);
        end
    end
    ...
    for i = 1:Nsubjects
        ...
        perf_max_u_j = perf_max_u(index{1:length(Lparam)});
        perf_max_u_j = perf_max_u_j(:);
    ...
    perf_mean_j = perf_mean(index{1:length(Lparam)});
    perf_mean_j = perf_mean_j(:);
    ax = sort(unique( CV_param(:, j) ));
...
for j = 1:length(Lparam)
    ...
    for i = 1:Nsubjects
        ...
        perf_u_j = perf_u(index{1:length(Lparam)});
        perf_u_j = perf_u_j(:);
    ...
    % Plot mean
    if length(Lparam) > 1
        perf_mean = reshape(perf_max, Lparam');   
    end
