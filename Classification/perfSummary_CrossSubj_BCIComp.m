...
inputFiles = dir(sprintf('%s%s/resultsCS/*.mat', rootPath, dataset));
...
for model = 1:Nmodels
    % Load model results
    nameFile = sprintf('%s%s/resultsCS/%s', rootPath, dataset, inputFiles(model,1).name);
    ...
    dataset_mean_perf = cat(2, dataset_mean_perf, mean_perf);
    
    method_labels{1, model} = ID;
end
dataset_max_mean_max_perf = mean(dataset_max_perf, 1);
dataset_max_mean_perf = max(dataset_mean_perf, [], 1);
dataset_max_mean_transfer = mean(dataset_max_perf(setdiff(1:Nsubjects, 1:sqrt(Nsubjects)+1:Nsubjects),:), 1);
%% Show dataset performnce - Table format


subject_labels = sprintf('%s ',NameSubjects{1,:});
subject_labels = strrep(subject_labels, '.mat', '');
subject_labels = [subject_labels 'MEAN' ' ' 'MEAN2' ' ' 'TMEAN'];
subject_labels = strsplit(subject_labels);

aux = round(10^decimals*[dataset_max_perf; dataset_max_mean_max_perf; ...
    dataset_max_mean_perf; dataset_max_mean_transfer])/10^decimals;
...
%% Show dataset performnce - Plot
figure(fig_number)
plot(dataset_mean_perf)
title(dataset)
legend( strrep(method_labels, '_', ':'))
