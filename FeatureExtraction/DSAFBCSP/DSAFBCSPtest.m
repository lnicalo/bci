% FBCSP
function features = DSAFBCSPtest(EEGdata, model)
% FBCSP
% 
% Default:
%
...
for l = 1:Nsegments    
    ...
    for j = 1:NtempFilters      
        ...
        Ctrain =  model.Cmean(:,:,j,l)^0.5;
        Ctest = squeeze(mean(scms(1:20,:,:)));
        for i = 20:Ntrials
            % Test covariance estimate
            Ctest = Ctest * i/(i+1) + squeeze(scms(i,:,:)/(i+1));
            
            V = Ctest^-0.5 *Ctrain;
            scms(i,:,:) = V'*squeeze(scms(i,:,:))*V;
        end
