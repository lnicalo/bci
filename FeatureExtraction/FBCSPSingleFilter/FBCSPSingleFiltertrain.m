% FBCSP
function [extractedFeatures, model] = FBCSPSingleFiltertrain(EEGdata, options)
% FBCSP
% 
% Default:
%
...
mi = NaN(Nfeatures, Nsegments);
reverseStr = '';
for i = 1:Nsegments
    for j = 1:Nfeatures
        f = squeeze(extractedFeatures(:, i, j));
        mi(j, i) = mutualInformation(f( EEGdata.validTrial == 1 ), EEGdata.trueLabel( EEGdata.validTrial == 1) );
    end
    
    % Display progress
    if options.display > 0
        percentDone = 100 * i / Nsegments;
        msg = sprintf('Computing mutual info: %3.1f\n', percentDone);
        fprintf([reverseStr, msg]);
        reverseStr = repmat(sprintf('\b'), 1, length(msg));
    end
end

mi = reshape( mi, [NtempFilters NspatialFilters Nsegments]);

%% Select spatial filters
[~, ind] = max(mi, [], 3);

selSpatialFilter = NaN(Nchannels, NspatialFilters, NtempFilters);
for i = 1:NtempFilters
    for j = 1:NspatialFilters
        selSpatialFilter(:, j, i) = squeeze( model.spatialFilter(:, j, i, ind(i,j) ) );
    end
end

%% Feature extraction with selected spatial filters
model.spatialFilter = selSpatialFilter;
extractedFeatures = NaN(Nfeatures, Nsegments, Ntrials);
