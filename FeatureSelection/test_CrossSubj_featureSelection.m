...
inputFiles = dir(sprintf('%s%s/testFeaturesCS/%s/none/*.mat', rootPath, dataset, extrFeatAlg));
Nsubjects = length(inputFiles);

for subject = 1:Nsubjects
    %% Load features
    nameFile = sprintf('%s%s/testFeaturesCS/%s/none/%s', rootPath, dataset, extrFeatAlg, inputFiles(subject,1).name);
    ...
    aux = strsplit(inputFiles(subject,1).name,'_');
    trainSubject = aux{1,2};
    namefile = sprintf('%s%s/trainFeatures/%s/%s/%s', rootPath, dataset, extrFeatAlg, featSelFolder, trainSubject);
    load(namefile,'model');
    ...
    %% Save
    out_dir = sprintf('%s%s/testFeaturesCS/%s/%s', rootPath, dataset, extrFeatAlg, featSelFolder);
